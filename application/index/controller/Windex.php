...
//        dump($result);
    ...
    public function userList()
    {
	    ...
	    $prefix=include '../application/mydatabase.php' ;
        $p = $prefix['prefix'];
...
//        $input = json_decode($input,1);
        ...
        $roleList =  Db::name('role')->page($currentPage,$pageSize)->select();
        $role['role'] = $roleList;
...
//        $input = json_decode($input,1);
        ...
        $input = input('get.');
        $rid = $input['rid'];
        $getPermission = $this->userModel->getMessg('role_permission',['rid' => $rid]);
        if($getPermission){
