...
class Websocket extends Controller 
{
    ...
    protected function getAutoChart( array $charts)
    {
        ...
        foreach($charts as $key => $value) {
            ...
            if( isset($config['dataOpt']['source']['type']) && isset($config['dataOpt']['autoUpdate'])) {
                //拿到是否自动刷新字段
                $isAotu = $config['dataOpt']['autoUpdate'];
                //数据源类型部位websocket
                $type = $config['dataOpt']['source']['type'];
                //判断是否自动刷新
                if($isAotu && strtolower($type != 'websocket')) {
                    //将自动刷新图表ID传入
                    $autu[$i]['tid'] = $value['tid'];
                    //自动刷新开始时间
                    $autu[$i]['autoupdatetime'] = $config['dataOpt']['clockStart'];
                    //自动刷新秒数
                    $autu[$i]['timeClock'] = $config['dataOpt']['timeClock'];
                    $i++;
                }
            }
        }
