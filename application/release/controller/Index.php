...
class Index extends Controller
{
    ...
    public function valiView()
    {
        ...
        //判断传值是否满足
        if($vali['err'] != 0) {
            return jsonRetuen(1,$vali['data']);
        }
        //查询相关发布
        $result = Db::name('publish')->where('scid',$input['pid'])->find();
        //判断查询是否为空
        if(empty($result)){
            return jsonRetuen(1,'未找到发布信息');
        }
        //判断是否需要验证token
        if(!empty($result['token'])){
            ...
            if(isset($input['token'])) {
                ...
                if($result['token'] != $input['token']) {
                    return jsonRetuen(1,'token验证失败');
                }
            }else {
                return jsonRetuen(1,'非法进入页面');
            }
        }
        //验证是否已过期
        if($result['expiredate'] < time() && $result['expiredate'] != 0) {
            return jsonRetuen(1,'发布链接已过期');
        }
