...
class User extends UserModel
{
    ...
    public function doLogin()
    {
        ...
        //解密
        $input['password'] = $this->decrypt($input['password'] , $input['len']);
        //查看浏览器是否拥有token
...
//            $online = $online+1;
            ...
            unset($result);
            $url = 'index';
            $data = ['token' => $account_session , 'path' => $url , 'user' => $user,'role' => $this->roleName];
    ...
    public function userPsswordUpdate()
    {
        ...
        //解密密码
        $input['password'] = $this->decrypt($input['password'] , $input['len']);
        $input['newPassword'] = $this->decrypt($input['newPassword'] , $input['newlen']);
        //获取uid
        $header = $this->get_all_header();
        $token = $header['token'];
        $getUid = $this->UserModel->getMessge('token' ,['token' => $token]);
    ...
    public function uploadImg()
    {
        ...
        if($input['type']){
            ...
            //文件夹路径
            $path = ROOT_PATH . 'public'  . DS . 'uploads' . DS . 'staticimg' . DS . date('Ymd' , time()) ;
            //判断目录是否存在 不存在就创建
            if (!is_dir($path)){
            ...
            //文件名字
            $fileName =  'Kwc' . date('Ymd' , time()) . mt_rand(999,9999).'.png';

            //文件路径
            $filePath = $path . DS . $fileName;

            //将图片写入文件
            $r = file_put_contents($filePath, base64_decode($file));

            if($r) {

                $avatarPath = '/uploads/staticimg/'.date('Ymd' , time()).'/'. $fileName;
                //文件路径入库
                $user = $this->UserModel->messgeUptate('user',['uid' => $uid] , ['avatar' => $avatarPath]);
