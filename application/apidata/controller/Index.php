...
use PHPExcel;
//use checkIp\ConvertIpNew\ipAddress;
header("ACCESS-CONTROL-ALLOW-ORIGIN:*");
header("Access-Control-Allow-Headers:token,username");
header('content-type:application/json;charset=utf8');
header('Access-Control-Max-Age: 86400');

function get_logdata($data,$startString,$endString){
	//去除指定字符串及之前的数据
	$returnData = substr($data,strpos($data,$startString)+strlen($startString));

	//截取以指定字符串结尾的数据 poller_item
	$return = substr($returnData,0,strpos($returnData,$endString));
...
function get_logip($log,$all){
		

		$patten = '/\d+\.\d+\.\d+\.\d+/';
		if(empty($all)){
			preg_match( $patten, $log, $out);
			if(empty($out)){
				return NULL;
			}
			$return = $out[0];
			...
			preg_match_all( $patten, $log, $return );
			return $return;
		}
...
class Index extends Controller 
{
	...
	// 	$data = Db::table('sdata_z_apache')->group('addr')->field('addr')->select();


	// 	for($i =0;$i<count($data);$i++){
	// 		$select = Db::table('sdata_d_coordinate')->where('name',$data[$i]['addr'])->find();
	// 		$upd['coordinate'] = $select['coordinate'];
	// 		$upd['class'] = $select['class'];

	// 		$update = Db::table('sdata_z_apache')->where('addr',$data[$i]['addr'])->update($upd);
	...
	// 	$data = Db::table('sdata_z_apache')->group('time')->select();


	// 	for($i =0;$i<count($data);$i++){
	// 	$return[$data[$i]['time']] = Db::table('sdata_z_apache')->where('time',$data[$i]['time'])->field('time,addr,count,coordinate,class')->select();
